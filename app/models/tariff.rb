...
class Tariff < ActiveRecord::Base
  ...
  def free_destinations_by_st(st, limit = nil, offset = 0)
    ...
    fdests = []
    fdests = adests - dests
    limit ? [fdests, actual_adest_count] : fdests
  end
...
WHERE rates.tariff_id = #{self.id} AND tmp_dest_groups.rate = ratedetails.rate
  ...
  def analize_file(name, options)
    ...
    if options[:imp_update_dest_names].to_i == 1 and options[:imp_dst] >= 0
      ...
      ActiveRecord::Base.connection.execute("UPDATE #{name} join destinations on (replace(col_#{options[:imp_prefix]}, '\\r', '') = destinations.prefix) SET ned_update = 1 WHERE BINARY replace(replace(TRIM(col_#{options[:imp_dst]}), '\r', ''), '  ', ' ') != IFNULL(IFNULL(original_destination_name,destinations.name),'')") if ActiveRecord::Base.connection.tables.include?(name)
    end
