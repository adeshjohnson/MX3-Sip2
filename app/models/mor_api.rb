  ...
  def MorApi.check_params_with_all_keys(params, request)
    #hack find user from params u and p
      ...
      ["server_ip", "device_type", "device_username", "device_password", "login_url", "login_username", "username", "first_name",
       ...
       "payment_currency", "payment_type", "payment_fee", "call_list", 'email_name', 'email_to_user_id', 'caller_id', 'device_id', "calldate", "source", "destination", "billsec", 'calls_string',
       'cli_number'].each { |key|
        ret[key.to_sym] = params[key.to_sym] if params[key.to_sym]
      }
	  
	  #New params for send_sms_verification_code. (u is not in hash)
	  
	  ret[:country_prefix] = params[:country_prefix].to_s if params[:country_prefix]
      ret[:local_number] = params[:local_number].to_s if params[:local_number]
	  
	  #New params for user_register_mobile. Some were already introduced by send_sms_verification_code (u is not in hash)
	  
	  ret[:verif_code] = params[:verif_code].to_s if params[:verif_code]
      ...
      hash_param_order = ['user_id', 'period_start', 'period_end', 'direction', 'calltype', 'device', 'balance',
        'monitoring_id', 'users', 'block', 'email', 'mtype', 'tariff_id', 'u0', 'u1', 'u2', 'u3', 'u4', 'u5', 'u6',
        'u7', 'u8', 'u9', 'u10', 'u11', 'u12', 'u13', 'u14', 'u15', 'u16', 'u17', 'u18', 'u19', 'u20', 'u21', 'u22',
        'u23', 'u24', 'u25', 'u26', 'u27', 'u28', 'ay', 'am', 'ad', 'by', 'bm', 'bd', 'pswd', 'user_warning_email_hour',
        'pgui', 'pcsv', 'ppdf', 'recording_forced_enabled', 'i4', 'tax4_enabled', 'tax2_enabled',
        'accountant_type_invalid', 'block_at_conditional', 'tax3_enabled', 'accountant_type', 'tax1_value',
        'show_zero_calls', 'warning_email_active', 'compound_tax', 'tax4_name', 'allow_loss_calls', 'tax3_name',
        'tax2_name', 'credit', 'tax1_name', 'total_tax_name', 'tax2_value', 'tax4_value', 'ignore_global_monitorings',
        'i1', 'tax3_value', 'cyberplat_active', 'i2', 'i3', 'recording_enabled', 'email_warning_sent_test',
        'own_providers', 'a0', 'a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8', 'a9', 's_user', 's_call_type',
        's_device', 's_provider', 's_hgc', 's_did', 's_destination', 'order_by', 'order_desc', 'only_did',
        'description', 'pin', 'type', 'devicegroup_id', 'phonebook_id', 'number', 'name', 'speeddial', 's_user_id',
        's_from', 'provider_id', 's_till', 's_transaction', 's_completed', 's_username', 's_first_name', 's_last_name',
        's_paymenttype', 's_amount_max', 's_currency', 's_number', 's_pin', 'p_currency', 'paymenttype',
        'tax_in_amount', 'amount', 'transaction', 'payer_email', 'fee', 'id', 'quantity', 'callerid', 'cardgroup_id',
        'status', 'date_from', 'date_till', 's_reseller_did', 's_did_pattern', 'lcr_id', 'dst', 'src', 'message',
        "server_ip", "device_type", "device_username", "device_password", "login_url", "login_username", "username",
        "first_name", "last_name", "full_name", "nice_balance", "warning_email_balance", "nice_warning_email_balance",
        "currency", "user_email", "company_email", "company", "primary_device_pin", "login_password", "user_ip", "date",
        "auth_code", "transaction_id", "customer_name", "company_name", "url", "trans_id", "cc_purchase_details",
        "monitoring_amount", "monitoring_block", "monitoring_users", "monitoring_type", "payment_amount",
        "payment_payer_first_name", "payment_payer_last_name", "payment_payer_email", "payment_seller_email",
        "payment_receiver_email", "payment_date", "payment_free", "payment_currency", "payment_type", "payment_fee",
        "call_list", 'email_name', 'email_to_user_id', 'caller_id', 'device_id', "calldate", "source", "destination",
        "billsec", 'calls_string', 'cli_number', 'country_prefix', 'local_number', 'verif_code']
