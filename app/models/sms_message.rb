...
class SmsMessage < ActiveRecord::Base
  ...
  def sms_status_code_tip
    case self.status_code.to_s
      when "0"
        out = "SMS is sent"
      when "1"
        out = "System owner does not have rate for this destination"
      when "2"
        out = "Reseller does not have rate for this destination"
      when "3"
        out = "User does not have rate for this destination"
      when "4"
        out = "Some error from provider"
      when "5"
        out = "Insufficient balance"
      when "6"
        out = "API returns no good keywords"
      when "001"
        out = "The message ID is incorrect or reporting isdelayed."
      when "002"
        out = "The message could not be delivered and has been queued for attempted redelivery."
      when "003"
        out = "Delivered to the upstream gateway or network(delivered to the recipient)."
      when "004"
        out = "Confirmation of receipt on the handset of the recipient."
      when "005"
        out = "There was an error with the message, probably caused by the content of the message itself."
      when "006"
        out = "The message was terminated by an internal mechanism."
      when "007"
        out = "An error occurred delivering the message to the handset."
      when "008"
        out = "Message received by gateway."
      when "009"
        out = "The routing gateway or network has had an error routing the message."
      when "010"
        out = "Message has expired before we were able to deliver it to the upstream gateway. No charge applies."
      when "011"
        out = "Message has been queued at the gateway for delivery at a later time (delayed delivery)."
      when "012"
        out = "The message cannot be delivered due to a lack of funds in your account. Please re-purchase credits."
      when "0001"
        out = "Authentication failed"
      when "0002"
        out = "Unknown username or password"
      ...
      when "0005"
        out = "Missing session ID"
      when "0007"
        out = "You have locked down the API instance to a specific IP address and then sent from an IP address different to the one you set."
      when "101"
        out = "Invalid or missing parameters"
      when "102"
        out = "Invalid user data header"
      ...
      when "108"
        out = "Invalid or missing API ID"
      when "109"
        out = "This can be either a client message ID or API message ID."
      when "110"
        out = "Error with email message"
      when "111"
        out = "Invalid protocol"
      when "112"
        out = "Invalid message type"
      when "113"
        out = "The text message component of the message is greater than the permitted 160 characters (70 Unicode characters)."
      when "114"
        out = "This implies that the gateway is not currently routing messages to this network prefix. Please email support@clickatell.com with the mobile number in question."
      when "115"
        out = "Message expired"
      when "116"
        out = "Invalid Unicode data"
      when "120"
        out = "Invalid delivery time"
      when "121"
        out = "This number is not allowed to receive messages from us and has been put on our block list."
      when "122"
        out = "Destination mobile opted out"
      when "123"
        out = "A sender ID needs to be registered and validated before it can be successfully used in message sending."
      when "201"
        out = "Invalid batch ID"
      when "202"
        out = "No batch template"
      ...
      when "302"
        out = "Max allowed credit"
      when "7"
        out = "Sending of Fake SMS was OK"
    end
    out
  end
  ...
  def sms_send(user, user_tariff, number, lcr, sms_numbers, message, options = {})
	...
	# Adesh
	cut_rule = provider.cut_rule.to_s
    add_rule = provider.add_rule.to_s
	mod_prefix=""
	if cut_rule.length > 0
		mod_prefix = number[0..cut_rule.length-1]
	end    
    new_prefix = mod_prefix.gsub(cut_rule,add_rule)
    number = new_prefix + number[cut_rule.length..number.length-1]	
	#
