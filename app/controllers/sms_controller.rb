...
class SmsController < ApplicationController
  ...
  before_filter :check_localization, :except => [:sms_result, :sms_list_registration]
  ...
  before_filter :check_sms_addon, :except => [:sms_result, :sms_list_registration]
  ...
  def sms_list_registration
	
    unless session[:usertype] == 'admin' or session[:usertype] == 'reseller'
      dont_be_so_smart
      redirect_to :controller => :callc, :action => :main and return false
    end
	
    @page_title = _('Sms_list_registration')
    #@page_icon = ""
    @page = 1
    @page = params[:page].to_i if params[:page]	
    @sms = SmsMessage.where("sms_type LIKE 'verification%' AND reg_owner = '#{session[:user_id]}'").order('sending_date DESC').all
