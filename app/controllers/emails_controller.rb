...
=end
  ...
  def EmailsController::send_invoices(email, to, from, files = [], number = 0)
     ...
     begin
	     files.each do |file| 
	       File.open("/home/mor/tmp/" + number.to_s + "_" + file[:filename].to_s.gsub(" ","_"), 'wb') {|f| f.write(file[:file]) }
	     end
...
=end
  ...
  def EmailsController.send_user_email_after_registration(user, device, password, reg_ip, free_ext, val_hash)
    if Confline.get_value("Send_Email_To_User_After_Registration") == "1"
      ...
      users = [user]
      variables = Email.email_variables(user, device, {:login_password => password, :user_ip => reg_ip, :val_hash => val_hash})
...
=end
  ...
  def EmailsController.send_user_email_after_payment(user, payment)
      email = Email.find(:first, :conditions => ["name = 'email_user_after_payment' AND owner_id= ?", user.owner_id])
      users = [user]
      variables = Email.email_variables(user, nil, payment)
      num = EmailsController.send_email(email, Confline.get_value("Email_from", user.owner_id), users, variables)
      return 1
  end
