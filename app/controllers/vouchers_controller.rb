...
class VouchersController < ApplicationController
  ...
  def voucher_pay
	...
	if !@user.email.blank?
		begin
				a = Thread.new {
				  send_email_to_user = EmailsController.send_user_email_after_payment(@user, {:payment_amount => p.amount, :payment_date => p.shipped_at, :payment_fee => p.fee, :payment_currency => p.currency, :payment_type => p.paymenttype})
				  #EmailsController.send_admin_email_after_registration(@user, device, params[:password], reg_ip, free_ext, owner.id)
				}
		rescue Exception => e
			  Action.add_action(@user.id, "Voucher", "Email not sent to user #{@user.username} after payment: #{p.amount} #{p.currency}")
		end
	else
			 Action.add_action(@user.id, "Voucher", "User #{@user.username} was not emailed after payment: #{p.amount} #{p.currency}. Email for is not registered.")
	end
