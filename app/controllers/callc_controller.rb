...
class CallcController < ApplicationController
  ...
  before_filter :authorize, :except => [:webphone, :webphone_date_limit, :webphone_invalid,:login, :try_to_login, :pay_subscriptions, :monthly_actions, :forgot_password, :email_validation, :send_sms_code, :get_prefix]
...
def get_prefix
	country = Direction.find(:first, :conditions => {:id => params[:country_id]})
	@dst= Destination.where("direction_code = '#{country.code}'").order("LENGTH(prefix)").first if country
	render :layout => false
end
  
def send_sms_code  # Params: number, country_id, reg_owner.
    @r = ""
	
	reg_owner = User.where(:uniquehash => params[:reg_owner]).first if params[:reg_owner] != ""
	if !reg_owner.blank? and (reg_owner.usertype == "admin" or reg_owner.usertype == "reseller") # This increases security
	if !params[:number].blank? and !params[:country_id].blank?
			country = Direction.find(:first, :conditions => {:id => params[:country_id]})
			dst = Destination.where("direction_code = '#{country.code}'").order("LENGTH(prefix)").first if country
			if dst	
				prefix = dst.prefix.to_s
				number = prefix + params[:number].to_s
				phone = Phonelib.parse(number)  #Parselib was uploaded as gem
					
				if phone.valid? and phone.types.include?(:mobile)
					extension = number
					devices = Device.where("username = '#{number}' OR extension = '#{extension}'").first
					dialplans = Dialplan.where("dptype = 'pbxfunction' and data2 = '#{extension}'").first
					if !devices and !dialplans #Send_sms
						if SmsMessage.where("number = '#{number}' AND sms_type = 'verification' AND user_price > 0").count < 4
							...
							if sms.save
								if Confline.get_value("Send_fake_SMS_for_mobile_validation", reg_owner.id).to_i == 0
									...
									if sms.status_code.to_s == "0"
									  @r = _('Success_sending_sms_check_code_operator', number.to_s, phone.carrier.to_s)

		


					else
								@r = _('Error_sending_sms_check_code')
								end	
								else
								sms.status_code = "7"
								sms.save
								@r = _('Ask_administrator_for_SMS_code')
								end
							else
							@r = _('Internal_error_unknown')
						end
						else
						@r = _('Limit_sms_reached')
					end					
					else
					@r = _('Mobile_phone_already_registered')		
				end
			else
				@r = _('The_number_is_not_a_mobile_number')
				end
			else
			@r = _('Country_not_found')
			end
		else
		@r = _('Please_enter_mobile_phone_and_select_country')
		end	
	else
		@r = _('Dont_be_so_smart')
	end
  ...
  def email_validation
	...
	unless params[:val_id] and params[:val_hash] and (user = User.where(:id => params[:val_id]).first)
		dont_be_so_smart
		redirect_to :controller => "callc", :action => "login" and return false
    end	
	
	unless (user.id.to_i != 0) and (user.validation_hash.to_i != 0) and (user.id.to_s == params[:val_id]) and (user.validation_hash == params[:val_hash])
		flash[:notice] = _('Email_Validation_Error')
		redirect_to :controller => "callc", :action => "login" and return false
	end
	user.blocked = 0
	user.validation_hash = '0'
	user.save
	flash[:status] = _('Email_Validation_Success')
	redirect_to :controller => "callc", :action => "login" and return false    			
  end
  ...
  def try_to_login
    ...
    if @user and @user.owner
      @login_ok = true
	  if @user.fulldata.to_i == 0
			make_default_user(@user)
	  end
      renew_session(@user)
      store_url
  ...
  def main
	...
	#START Added by Adesh Johnson
	@spay_enabled = session[:safetypay_enabled]
	@balotoc_enabled = session[:balotociti_enabled]  	


	@mercadopago_enabled = session[:mercadopago_enabled]  
	#END Added by Adesh Johnson
  ...
  def signup_end
    ...
    session[:reg_mob_phone] = params[:mob_phone].to_s.gsub(/[^0-9]/, "")
	params[:mob_phone_orig] = session[:reg_mob_phone]
	
    session[:reg_fax] = params[:fax]
	session[:reg_verif_code] = params[:verif_code]  #
	

	# Adesh
	params[:country_id] = Confline.get_value("Default_Country_ID").to_i if params[:country_id].blank?
	country = Direction.find(:first, :conditions => {:id => params[:country_id]})
	params[:country_id] = country.id.to_s
	dst = Destination.where("direction_code = '#{country.code}'").order("LENGTH(prefix)").first if country
	prefix = dst.prefix.to_s
	params[:mob_phone] = (prefix + params[:mob_phone_orig].to_s) if params[:mob_phone_orig].to_s !=""
	params[:phone] =  (prefix + params[:phone].to_s) if params[:phone].to_s !=""
    ...
    if capt and !notice or notice.blank?
	  ...
	  params[:caller_name] = params[:email] if params[:email]!=""
      @user, @send_email_to_user, @device, notice2 = User.create_from_registration(params, @owner, reg_ip, params[:extension].to_s, new_device_pin(), random_password(8), next_agreement_number)  #Adesh
  ...
  def make_default_user(user)
		...
		owner = user.owner_id
		#read_attribute is used to avoid overwritting by model class (read directly from DB)
		email = user.read_attribute('email')
		user_default = Confline.get_default_object(User, owner)
		
		#Overwritting current user with default user's values. Add additional values in future (maybe when other default parameters be set)

		user.warning_email_active = user_default.warning_email_active
		user.recording_enabled = user_default.recording_enabled
		user.postpaid = user_default.postpaid
		user.recording_forced_enabled = user_default.recording_forced_enabled
		user.language = user_default.language
		user.lcr_id = user_default.lcr_id
		user.vat_number = user_default.vat_number
		user.credit = user_default.credit
		user.allow_loss_calls = user_default.allow_loss_calls
		user.warning_email_balance = user_default.warning_email_balance
		user.tariff_id = user_default.tariff_id
		user.invoice_zero_calls = user_default.invoice_zero_calls
		user.block_at = user_default.block_at
		user.blocked = user_default.blocked
		user.taxation_country = user_default.taxation_country
		user.usertype = user_default.usertype
		user.block_conditional_use = user_default.block_conditional_use
		user.cyberplat_active = user_default.cyberplat_active
		user.acc_group_id = user_default.acc_group_id
		user.generate_invoice = user_default.generate_invoice
		user.recording_hdd_quota = user_default.recording_hdd_quota
		user.accounting_number = user_default.accounting_number
		user.call_limit = user_default.call_limit
		user.send_invoice_types = user_default.send_invoice_types
		user.hidden = user_default.hidden
		user.balance = user_default.balance
		user.currency_id = user_default.currency_id
		user.time_zone = user_default.time_zone
		user.responsible_accountant_id = user_default.responsible_accountant_id
		user.warning_email_hour = user_default.warning_email_hour
		user.warning_balance_call = user_default.warning_balance_call
		user.warning_balance_sound_file_id = user_default.warning_balance_sound_file_id
		#user.recordings_email = user_default.recordings_email
		user.ignore_global_monitorings = user_default.ignore_global_monitorings
		user.hide_destination_end = user_default.hide_destination_end
		user.minimal_charge = user_default.minimal_charge
		user.minimal_charge_start_at = user_default.minimal_charge_start_at
	
		user.agreement_date = Time.now().to_s(:db)
		user.recordings_email = email
		#for backwards compatibility - user had no address or tax before, so let's give it/them to him
		unless user.address
		...
		user.fulldata = 1
		if user.save and user.address.save and user.tax.save
			user.create_balance_payment
		end
	end
