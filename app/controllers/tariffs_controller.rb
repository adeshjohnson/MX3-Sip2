...
=end
  ...
  def rate_new_by_direction_add
    ...
    @destinations.each { |dest|
      if params["dest_#{dest.id}"] and params["dest_#{dest.id}"].to_s.length > 0
        @tariff.add_new_rate(dest.id, params["dest_#{dest.id}"], 1, 0,0, params[('gh_'+dest.id.to_s).intern])
      end
    }
  ...
  def rate_try_to_add
    ...
    for dest in @tariff.free_destinations_by_st(st)
      ...
      if params[(dest.id.to_s).intern].to_s.length > 0
        @tariff.add_new_rate(dest.id.to_s, params[(dest.id.to_s).intern], 1, 0,0, params[('gh_'+dest.id.to_s).intern])
      end
    end
  ...
  def dst_to_update_from_csv
    ...
    if @csv2.to_i == 0
      ...
      if ActiveRecord::Base.connection.tables.include?(session["tariff_name_csv_#{params[:tariff_id].to_i}".to_sym])
        @dst = ActiveRecord::Base.connection.select_all("SELECT destinations.prefix, col_#{session["tariff_import_csv2_#{@tariff_id}".to_sym][:imp_dst]} as new_name, IFNULL(original_destination_name,destinations.name) as dest_name FROM destinations JOIN #{session["tariff_name_csv_#{params[:tariff_id].to_i}".to_sym]} ON (replace(col_#{session["tariff_import_csv2_#{@tariff_id}".to_sym][:imp_prefix]}, '\\r', '') = prefix) WHERE ned_update IN (1, 3, 5, 7) AND BINARY replace(replace(TRIM(col_2), '\r', ''), '  ', ' ') != IFNULL(IFNULL(original_destination_name,destinations.name),'')")
      end
    end
    render(:layout => "layouts/mor_min")
  end
