...
=end
  ...
  def create
	...
	# START ... Adesh
	@user.send_balance_to_others = params[:send_balance_to_others].to_i
	
	@user.fulldata = 1
		# For Joomla-related fields
  ...
  def destroy
    ...
    if params[:id].to_i != 0
      response = user.destroy_everything
	  if response == 1
		flash[:status] = _('User_deleted')
	  else
	    flash[:status] = _('User_not_deleted')
	  end
    else
