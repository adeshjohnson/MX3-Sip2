...
=end
  ...
  def renew_session(user)
	...
	session[:safetypay_enabled] = Confline.get_value("SafetyPay_Enabled", user.owner_id).to_i
	session[:balotociti_enabled] = Confline.get_value("BalotoCiti_Enabled", user.owner_id).to_i
	session[:mercadopago_enabled] = Confline.get_value("MercadoPago_Enabled", user.owner_id).to_i
	#END Added by Adesh
...
=end
  ...
  def nice_session
    ...
    [:username, :first_name, :last_name, :user_id, :usertype, :owner_id, :tax, :usertype_id, :device_id, :tariff_id, :sms_service_active, :user_cc_agent,
     ...
     :active_calls_refresh_interval, :show_full_src, :show_rates_for_users, :webmoney_enabled,  :safetypay_enabled, :balotociti_enabled, :mercadopago_enabled, :paypal_enabled,
